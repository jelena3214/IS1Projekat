...
public class CommandExecuter {
    ...
    private Serializable createClient(CreateClientCommand createCmd, EntityManager em) {
        ...
        if(grad.getResultList().isEmpty()){
            System.out.println("Ne postoji dati grad u bazi");
            return new TextResponse("Ne postoji dati grad u bazi");
        }
        
        TypedQuery<Korisnik> korisnikq = em.createNamedQuery("Korisnik.findByUsername",Korisnik.class).setParameter("username", createCmd.getUsername());
        
        if(!korisnikq.getResultList().isEmpty())return new TextResponse("Postoji korisnik sa datim username-om");
        ...
        Grad gr = grad.getResultList().get(0);
    ...
    private Serializable addClientMoney(AddClientMoneyCommand addCmd, EntityManager em) {
        ...
        if(korisnik.getResultList().isEmpty()){
        ...
        if(addCmd.getNovac() < 0){
            return new TextResponse("Uneli ste negativan broj za dodavanje novca");
        }
        
        Korisnik kor = korisnik.getResultList().get(0);
    ...
    private Serializable changeCityAdress(ChangeAdressCityCommand changeCmd, EntityManager em) {
        ...
        Grad noviGrad = grad.getResultList().get(0);
